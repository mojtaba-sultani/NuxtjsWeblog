        ...
        <div class="container w-full lg:w-[78%] xl:w-[67%] mt-32">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <NuxtLink :to="`/shortCard/${comment.id}`" v-else v-for="comment in Comments" :key="comment.id"
                    ...
                    <h4 class="font-bold text-red-500">id:{{ comment.id }}</h4>
                    <h4 class="font-semibold text-justify"><span class="text-violet-500 font-bold">name:</span> {{
                        comment.name }}</h4>
                    <p class="text-gray-500"><span class="text-blue-600 font-bold me-1 text-justify">email:</span>{{
                        comment.email }}</p>
                </NuxtLink>
            </div>
